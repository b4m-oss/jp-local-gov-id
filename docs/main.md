...
| パッケージ | 内容 | パス |
|------------|------|------|
| JS API パッケージ | 検索・コード解決などの API | `packages/jp-local-gov-id`（npm: `jp-local-gov-id`） |
| データ JSON パッケージ | 分割された公式データ（後述） | `packages/jp-local-gov-id-data`（npm: `jp-local-gov-id-data`） |
...
import * as dataset from "jp-local-gov-id-data";
