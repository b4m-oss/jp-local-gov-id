...
| パッケージ | 説明 | バージョン |
|------------|------|------------|
| [`jp-local-gov-id`](./packages/jp-local-gov-id) | JS API（データ非同梱・遅延ロード） | 0.3.0 |
| [`jp-local-gov-id-data`](./packages/jp-local-gov-id-data) | 分割データ JSON | 0.2.0 |

## インストール（利用側）

```bash
# API + 公式データ（npm から import して渡す場合）
npm install jp-local-gov-id jp-local-gov-id-data

# API のみ（版付きインデックス URL から取得する場合）
npm install jp-local-gov-id
```
...
```ts
import { createLocalGov } from "jp-local-gov-id";
import dataset from "jp-local-gov-id-data";
...
- API パッケージには JSON を同梱しません（`jp-local-gov-id-data` または URL で渡してください）
